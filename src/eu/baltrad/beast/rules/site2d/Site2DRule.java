...
public class Site2DRule implements IRule, InitializingBean {
  ...
  public IBltMessage handle(IBltMessage message) {
    ...
    if (message instanceof BltDataMessage) {
      ...
      logger.debug("ENTER: execute Site2DRule with ruleId: " + getRuleId() + ", thread: " + Thread.currentThread().getName() + 
          ", file: " + file.getUuid());
      ...
      logger.debug("EXIT: execute Site2DRule with ruleId: " + getRuleId() + ", thread: " + Thread.currentThread().getName() + 
          ", file: " + file.getUuid());
